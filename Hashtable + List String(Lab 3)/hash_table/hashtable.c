...
{
	int table_index = hash_string(item, this -> size);
	listset_add(this -> table[table_index],item);
}
...
{
	int table_index = hash_string(item,this -> size);
	return listset_lookup(this -> table[table_index],item);
}
...
{
	int table_index = hash_string(item,this -> size);
	listset_remove(this -> table[table_index],item);
}
