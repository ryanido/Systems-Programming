...
struct bloom * bloom_new(int size) {
  struct bloom * this = malloc(sizeof(struct bloom));
  this -> size = size;
  this -> bitset = bitset_new(size);
  return this;
}

// check to see if a string is in the set
int bloom_lookup(struct bloom * this, char * item) {
  int tmp_hash = hash_string(item,BLOOM_HASH1,this -> size);
  int tmp_hash2 = hash_string(item,BLOOM_HASH2,this -> size);
  struct bitset * set = this -> bitset;
  if(bitset_lookup(set,tmp_hash) && bitset_lookup(set,tmp_hash2))
  {
    return 1;
  }
  else
...
void bloom_union(struct bloom * dest, struct bloom * src1,
    struct bloom * src2) {
      struct bitset*tmp = src1 -> bitset;
      struct bitset*tmp2 = src2 -> bitset;
      bitset_union(dest -> bitset, tmp,tmp2);
}
...
void bloom_intersect(struct bloom * dest, struct bloom * src1,
    struct bloom * src2) {
      struct bitset*tmp = src1 -> bitset;
      struct bitset*tmp2 = src2 -> bitset;
      bitset_intersect(dest -> bitset, tmp,tmp2);
}
